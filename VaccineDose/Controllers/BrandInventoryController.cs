        ...
        public Response<List<BrandInventoryDTO>> Get(int Id)
        {
            try
            {
                using (VDConnectionString entities = new VDConnectionString())
                {
                    ...
                    List<BrandInventory> brandInventoryDBs = entities.BrandInventories.Where(x => x.DoctorID == Id).ToList();
                    if (brandInventoryDBs == null || brandInventoryDBs.Count() == 0)
                        return new Response<List<BrandInventoryDTO>>(false, "inventory not found", null);

                    List<BrandInventoryDTO> BrandInventoryDTOs = Mapper.Map<List<BrandInventoryDTO>>(brandInventoryDBs);
                    return new Response<List<BrandInventoryDTO>>(true, null, BrandInventoryDTOs);
                }
            }
            catch (Exception e)
            {
